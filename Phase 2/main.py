...
for i in range(num_train_samples):
    ...
    mnist_train_images[i] = dataset_train[i][0].flatten().numpy().astype(np.int64)
...
fc3 = PktFc(fc_dim2, num_classes, use_dfa=True, activation=activation)

fc1.set_next_layer(fc2)
fc2.set_next_layer(fc3)

train_target_mat = np.zeros((num_train_samples, num_classes))
train_target_mat.data[r][train]
test_target_mat = np.zeros((num_test_samples, num_classes))
