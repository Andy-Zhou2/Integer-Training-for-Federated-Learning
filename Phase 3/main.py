...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        train(self.net, self.trainloader, epochs=1, config=config | {'cid': self.cid})
        return get_parameters(self.net), len(self.trainloader), {}

    def evaluate(self, parameters, config):
        # print(f'evaluating {self.cid} with config {config}')
        set_parameters(self.net, parameters)
        loss, accuracy = test(self.net, self.valloader, config | {'cid': self.cid})
...
def federated_evaluation_function(server_round: int, parameters, fed_eval_config):
    ...
    loss, accuracy = test(net, testloader, fed_eval_config | {'cid': 'server', 'server_round': server_round})
