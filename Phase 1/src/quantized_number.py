...
class TestQuantizedArray(unittest.TestCase):
    ...
    def test_creation(self):
        ...
        qa = QuantizedArray.quantize([1, 2, 3], scale=2 ** -10, zero_point=0, bit_width=64)
        self.assertTrue(np.array_equal(qa.array, np.array([1, 2, 3])* 2**10))

    def test_bit_width_acc_loss(self):
        qa = QuantizedArray.quantize([1, 2, 3], scale=2 ** -10, zero_point=0, bit_width=10)
        self.assertTrue(np.array_equal(qa.array, np.array([511, 511, 511])))

        qa = QuantizedArray.quantize([1, 2, 3], scale=2 ** -10, zero_point=0, bit_width=11)
        self.assertTrue(np.array_equal(qa.array, np.full([3], 1023)))

        qa = QuantizedArray.quantize([1, 2, 3], scale=2 ** -10, zero_point=0, bit_width=12)
        self.assertTrue(np.array_equal(qa.array, np.array([1024, 2047, 2047])))


    def test_bit_width(self):
        # Test if the bit_width is correctly set
        qa = QuantizedArray.quantize([1, 2, 3], scale=2 ** -10, zero_point=0, bit_width=8)
        self.assertEqual(qa.bit_width, 8)

    def test_dequantize(self):
        arr = [1, 2, 3]
        qa = QuantizedArray.quantize(arr, scale=2 ** -10, zero_point=0, bit_width=64)
        ...
        arr = [-2, -5, -7, 10]
        qa = QuantizedArray.quantize(arr, scale=2 ** -10, zero_point=2**30, bit_width=64)
        self.assertTrue(np.allclose(qa.dequantize(), arr))

    def test_add(self):
        arr1 = np.array([1, 2, 3])
        arr2 = np.array([3, 4, 5])
        a = QuantizedArray.quantize(arr1, scale=2 ** -10, zero_point=2**30, bit_width=64)
        b = QuantizedArray.quantize(arr2, scale=2 ** -10, zero_point=2**30, bit_width=64)
...
class TestQuantizedArrayMatMul(unittest.TestCase):
    ...
    def test_matmul_basic(self):
        ...
        qa1 = QuantizedArray.quantize(arr1, scale, zero_point, bit_width)
        qa2 = QuantizedArray.quantize(arr2, scale, zero_point, bit_width)
    ...
    def test_matmul_identity(self):
        ...
        qa = QuantizedArray.quantize(arr, scale, zero_point, bit_width)
        qa_identity = QuantizedArray.quantize(identity, scale, zero_point, bit_width)
    ...
    def test_matmul_zero_matrix(self):
        ...
        qa = QuantizedArray.quantize(arr, scale, zero_point, bit_width)
        qa_zero = QuantizedArray.quantize(zero_matrix, scale, zero_point, bit_width)
    ...
    def test_matmul_different_bit_width(self):
        ...
        qa1 = QuantizedArray.quantize(arr1, scale, zero_point, bit_width_1)
        qa2 = QuantizedArray.quantize(arr2, scale, zero_point, bit_width_2)
